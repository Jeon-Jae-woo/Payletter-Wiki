...
**🌐 배포 주소: [payletter-wiki.vercel.app](https://payletter-wiki.vercel.app)**
...
### 4.4 문서 공유 (PRD 미구현 → 구현 완료)

| 기능 | 구현 여부 | 비고 |
|---|:---:|---|
| 공개 URL로 문서 공유 | ✅ | 에디터 "공유" 버튼 → `/share/{id}` 공개 링크 발급 |
| 비로그인 열람 | ✅ | Supabase anon RLS 정책 + proxy 예외 처리 |
| 읽기 전용 뷰 | ✅ | TipTap `editable: false` 모드, 아이콘·커버·제목 표시 |
| 보안 제약 | ✅ | 비공개 문서 공유 불가, 공유 중 비공개 전환 불가 |
...
| Deployment | (미정의) | Vercel — payletter-wiki.vercel.app | 2026-03-25 배포 완료 |
...
|---|---|---|
| [System-Architect] | 전체 프로젝트 스캐폴딩, 폴더 구조 설계, 패키지 의존성 관리 | `src/` 디렉토리 기반 App Router 구조 설계, `(auth)` / `(wiki)` / `share` Route Group 분리, `types/`, `hooks/`, `lib/` 레이어 설계, 패키지 의존성 확정 |
| [DB-Agent] | DB 스키마 설계, SQL 마이그레이션, RLS 설정, API Route 최적화 | `documents`, `calendar_events`, `event_document_links`, `todos` 4개 테이블 설계, RLS 적용, anon 공유 정책(`007_public_share.sql`) 추가, `supabase/migrations/` 7개 마이그레이션 파일 작성, `lib/documents.ts` · `lib/calendar.ts` · `lib/todos.ts` 데이터 접근 레이어 구현 |
| [UI-Agent] | Shadcn UI 커스텀, Payletter Blue 테마 적용, Notion 스타일 레이아웃 구현 | `globals.css`에 Payletter Blue 테마 및 TipTap prose 스타일 적용, Pretendard 폰트 CDN 연동, `MainLayout.tsx` / `Sidebar.tsx` 구현, `ShareView.tsx` 공유 읽기 전용 뷰 구현 |
| [Feature-Agent] | 에디터 로직 구현, 캘린더 연동, 복잡한 비즈니스 로직 처리 | TipTap 에디터(암호화·아이콘·커버·즐겨찾기·체크박스·공유), 슬래시 커맨드, `useAutoSave.ts`, 캘린더 뷰, 통합 검색 모달, AES-256-GCM 암호화, 사이드바 실시간 동기화, 할 일 페이지, 공유 기능(`/share/[id]`) |
...
| 문서 공유 (`/share/[id]`) | 공개 URL로 비로그인 열람 가능. Supabase anon RLS + 읽기 전용 TipTap 렌더러 |
| 사이드바 실시간 동기화 | 제목·아이콘·공개여부 변경 시 CustomEvent로 사이드바 즉시 갱신 |
...
| 일정 상세 모달 | 하단 바 → 화면 중앙 모달로 변경, 연결 문서 목록 표시 |
| 페이지 백링크 | `/페이지 링크` 슬래시 커맨드 → `PageSearchModal`에서 문서 검색 후 인라인 칩으로 삽입 |
| 다크모드 | `useTheme` 훅 + `ThemeProvider`, localStorage 저장, 깜빡임 방지 인라인 스크립트 |
| Vercel 배포 | payletter-wiki.vercel.app — 2026-03-25 배포 완료 |
...
| Wiki | 멀티미디어 블록 | 동영상 임베드, 코드 블록 하이라이팅 |

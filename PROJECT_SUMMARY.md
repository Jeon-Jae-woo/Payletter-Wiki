...
페이레터(Payletter)의 브랜드 컬러(`#0054FF`)와 노션(Notion)의 UX를 결합한 개인형 위키 + 캘린더 통합 지식 관리 플랫폼으로, 문서와 일정 사이의 백링크(Backlink)로 컨텍스트를 연결한다.
...
| Editor | TipTap (Rich Text & Slash Command 지원) | TipTap v3 (Rich Text + 슬래시 커맨드) | 버전 명시 |
...
| [DB-Agent] | DB 스키마 설계, SQL 마이그레이션, RLS 설정, API Route 최적화 | `documents`, `calendar_events`, `event_document_links` 3개 테이블 설계, RLS 적용, `supabase/migrations/001_init_schema.sql` 작성, `lib/documents.ts`(9개 함수) 및 `lib/calendar.ts` 데이터 접근 레이어 구현, `types/database.ts` 제네릭 타입 정의 |
| [UI-Agent] | Shadcn UI 커스텀, Payletter Blue 테마 적용, Notion 스타일 레이아웃 구현 | `globals.css`에 Payletter Blue 테마 및 TipTap prose 스타일 적용, Pretendard 폰트 CDN 연동, `MainLayout.tsx` / `Sidebar.tsx` 구현, 인증 페이지 레이아웃, Shadcn UI 컴포넌트(`Button`, `Input`, `ScrollArea`, `Separator`, `Tooltip`) 구성 |
| [Feature-Agent] | 에디터 로직 구현, 캘린더 연동, 복잡한 비즈니스 로직 처리 | TipTap 에디터 구현(`Editor.tsx`), 슬래시 커맨드 익스텐션 및 팝업 메뉴(`SlashCommandExtension.ts`, `SlashCommandMenu.tsx`), 아이콘/커버/즐겨찾기 기능, `useAutoSave.ts` 훅, 캘린더 뷰(`CalendarView.tsx`), 일정 생성 모달(`EventCreateModal.tsx`), 통합 검색 모달(`SearchModal.tsx`) |
...
| 슬래시 커맨드 7가지 블록 타입 | 제목 1~3, 본문, 글머리 기호, 순서 목록, 인용구 — 구체적 블록 종류를 PRD에서 미정의 |
| tippy.js 의존성 | 슬래시 커맨드 팝업 메뉴 렌더링을 위해 추가 |
| `sort_order` 필드 | 문서 정렬 순서를 저장하는 컬럼 (`documents` 테이블) |
| `all_day` / `color` 필드 | 종일 여부 및 이벤트 색상 컬럼 (`calendar_events` 테이블) |
| `Supabase` 데이터베이스 제네릭 타입 | `types/database.ts`로 타입 안전성 강화 |

...
Vision: 페이레터(Payletter)의 신뢰감 있는 브랜드 컬러와 노션(Notion)의 직관적인 UX를 결합한 개인형 지식 및 일정 관리 플랫폼.

Goal: 단순한 메모를 넘어, 위키 문서와 캘린더 일정이 상호 연결되는 지능형 워크스페이스 구축.
...
Sidebar: 가변형 좌측 네비게이션 (문서 트리 구조).

Main Content: 가독성을 극대화한 중앙 집중형 에디터 영역.
...
5. Agent Collaboration Protocol (Sub-agent Architecture)
...
Context Load: 메인 에이전트는 모든 작업 전 PRD.md를 재독하여 방향성을 확인한다.

Auto-Delegation: 작업 성격에 맞는 서브에이전트를 선언하고 작업을 시작한다. (예: "지금부터 [UI-Agent] 모드로 사이드바를 만듭니다.")

Validation: 작업 후 결과물이 페이레터 브랜드 가이드와 노션의 사용성을 충족하는지 검토한다.

Git Commit: 작업 단위별로 Conventional Commits 규칙에 따라 커밋을 수행한다.